 ...
 * @copyright 2020 Irfan Ghuori
 * @license   MIT License
 * @version   2.0
 * @website   https://github.com/IrfanGhuori
 */

session_start();
header('Content-type:application/json');

class MakeConnectionToDatabase
{
/*** MYSQL HOSTNAME ***/
...
public $error_message = null;
    ...
    try{
        $this->db_host = $data["hostname"];
        $this->db_name = $data["databasetitle"];
        $this->db_user = $data["databasename"];
        $this->db_pass = $data["databasepass"];
        $this->token = $data["access_token"];
        $this->conn = new PDO('mysql:host=' . $this->db_host .';dbname=' . $this->db_name, $this->db_user, $this->db_pass);
        $this->conn->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        
        /*** echo a message saying we have connected ***/
        echo json_encode(["status" => true, "message" => "Connected successfully"]);
        $_SESSION["hostname"] = $this->db_host;
        $_SESSION["dbname"] = $this->db_name;
        $_SESSION["dbuser"] = $this->db_user;
        $_SESSION["dbpass"] = $this->db_pass;
    ...
    }catch(PDOException $error)
    {
        ...
        { if($dx == $codes): echo  json_encode(["status" => false, "message" => $string]); endif; }
        }
    }
    }
...
if(isset($_POST["host_title"]))
{
    $arr = array(

        "hostname" => $_POST["host_title"],
        "databasename" => $_POST["data_user"],
        "databasepass" => $_POST["data_pass"],
        "databasetitle" => $_POST["data_title"],
        "access_token" => $_POST["api_key"]
    );
    $dns = new MakeConnectionToDatabase();
    $dns->CreateConnection($arr);
}
?>